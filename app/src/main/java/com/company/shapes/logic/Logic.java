...
public class Logic 
    ...
    public static double rightTrianglePerimeter (double base, double height){
        double x;
        x = Math.sqrt(Math.pow(height, 2) + Math.pow(base, 2));
        return base + height + x;
    }

    public static double boxVolume (double lenght, double width, double depth) {
        return lenght * width * depth;
    }

    public static double boxSurfaceArea (double lenght, double width, double depth) {
        return (lenght * width + width * depth + lenght * depth) * 2;
    }
